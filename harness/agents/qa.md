...
name: qa
description: >-
  Tester. Fills gaps in the test plan and tests every feature when it is finished or fixed. Spawn after
  a feature merges (post Gate C) and after each bug fix. A feature is verified only after QA passes it,
  never when the dev merges. Files bugs with repro, expected versus actual, and severity.
model-tier: standard
claude-tools: Read, Bash, Glob, Grep, Write, Edit
opencode-mode: subagent
opencode-permission: edit=allow, bash=allow, read=allow
---
